...
bool Tuple::IsPoint() const { return epsilon_eq(1.0, w_); }

bool Tuple::IsVector() const { return epsilon_eq(0.0, w_); }
...
Tuple Tuple::Normalized() const {
  ...
  return Tuple(this->x() / magnitude, this->y() / magnitude,
...
Tuple Vector(float x, float y, float z) { return Tuple(x, y, z, 0.0); }

void Tuple::Debug() const { std::cout << DebugString() << std::endl; }
...
bool operator==(const Tuple lhs, const Tuple rhs) {
  return epsilon_eq(lhs.x(), rhs.x()) && epsilon_eq(lhs.y(), rhs.y()) &&
         epsilon_eq(lhs.z(), rhs.z()) && epsilon_eq(lhs.w(), rhs.w());
}
